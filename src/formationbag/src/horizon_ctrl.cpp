...
geometry_msgs::Point uav2_relapose(geometry_msgs::PoseStamped uav1_now)
{
    ...
    uav2_goal.x = uav1_now.pose.position.x + d;
...
geometry_msgs::Point uav3_relapose(geometry_msgs::PoseStamped uav1_now)
{
    ...
    uav3_goal.x = uav1_now.pose.position.x + 2*d;
...
{
    ros::init(argc,argv,"horizon_ctrl");
    ros::NodeHandle nh;

    //订阅终端的param话题  <formationbag::Point4>
    ros::Subscriber paramsub = nh.subscribe("/PC/horizon_param", 1000, paramCallback); 
