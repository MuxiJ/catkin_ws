...
void centerCallback(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
    ...
    //当uav1到了后，订阅到triangle_param时才发布位置,否则飞机为静止   
    if((flag==true) && pow(delta_x,2) + pow(delta_y,2) + pow(delta_z,2) < 0.01)
    {
        ...
        ROS_INFO_STREAM("triangle  r="<<r<<"  rate="<<rate);
    }
    flag = false;
}
...
{
    flag = true;//终端必须得一直发triangle使得flag一直1（-r 10）
    r = msg->r;
    rate = msg->rate;
}
...
{
    ros::init(argc, argv, "triangle_ctrl");
    ros::NodeHandle nh;
    ros::Time time0 = ros::Time::now();

    //订阅终端的param话题  <formationbag::Point5>
    ros::Subscriber paramsub = nh.subscribe("/PC/triangle_param", 1000, paramCallback); 
